...
app.post('/api/clients', requireAuth, async (req, res) => {
  try {
    const { name, code, cnpj, email, days_default, price_table, obs_default } = req.body;
...
// ---- ORDERS ----
const ORDER_FIELDS = ['boi_cas','nov_cas','boi_ts','boi_tcc','boi_dtb','boi_pab','vac_cas','vac_ts','vac_tcc','vac_dtv','vac_pav','fig','rab','buc','cor','cup','san','lom','dia','ind','lingua','cfile','alcatra','maminha','filet45','filetbc','coxmole','coxduro','patinho','lagarto','capafile','musculo','musc_mole','musc_duro','picanha_r','diaf_bloco','filet_bica'];
...
app.get('/api/orders/:weekLabel', requireAuth, async (req, res) => {
  try {
    ...
    orders.forEach(o => {
      ...
      // Merge with existing entry if duplicate rows exist
      if(result[key]) {
        const existing = result[key];
        // Keep _obs from whichever row has it
        if(!entry._obs && existing._obs) entry._obs = existing._obs;
        // Keep higher qty values
        ORDER_FIELDS.forEach(f => {
          if((!entry[f] || entry[f]===0) && existing[f]) entry[f] = existing[f];
        });
        // Merge all _price_ fields from existing
        Object.keys(existing).filter(k=>k.startsWith('_price_')||k.startsWith('_qty2_')||k.startsWith('_price2_')).forEach(k=>{
          if(!entry[k]) entry[k] = existing[k];
        });
      }
      result[key] = entry;
    });
...
app.post('/api/orders/:weekLabel', requireAuth, async (req, res) => {
  try {
    ...
    const days = daysRaw 
      ...
      : '14 Dias';
    // Extract custom prices, qty2, price2 and obs
    const prices = {};
    Object.keys(qtd).filter(k=>
      k.startsWith('_price_') || k.startsWith('_qty2_') || 
      k.startsWith('_price2_') || k==='_obs'
    ).forEach(k=>{ 
      if(k==='_obs') { if(qtd[k]!==undefined) prices[k]=qtd[k]; } // keep even if ''
      else if(qtd[k]!==undefined && qtd[k]!=='') prices[k]=qtd[k];
    });
    const pricesJson = JSON.stringify(prices);
    // Save order — check existing then insert or update
    const existing = await db.prepare(
      'SELECT id FROM orders WHERE client_id = ? AND week_label = ?'
    ).get(String(clientId), week);
    if(existing){
      await db.prepare(
        `UPDATE orders SET ${ORDER_FIELDS.map(f=>f+'=?').join(',')},days=?,prices_json=? WHERE client_id=? AND week_label=?`
      ).run(...vals, days, pricesJson, String(clientId), week);
    } else {
      await db.prepare(
        `INSERT INTO orders (client_id,week_label,${ORDER_FIELDS.join(',')},days,prices_json) VALUES (?,?,${ORDER_FIELDS.map(()=>'?').join(',')},?,?)`
...
app.get('/api/snapshots', requireAuth, async (req, res) => {
  try {
    const rows = await db.prepare(
      'SELECT id, label, week_label, created_at FROM snapshots ORDER BY created_at DESC'
    ).all();
    res.json(rows);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/snapshots/:id', requireAuth, async (req, res) => {
  try {
    ...
    if(!row) return res.status(404).json({ error: 'not found' });
    res.json({ ...row, snapshot_data: JSON.parse(row.snapshot_data) });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/debug/orders', requireAuth, async (req, res) => {
  try {
    const rows = await db.prepare('SELECT client_id, week_label, boi_cas, nov_cas, vac_cas, days, prices_json FROM orders ORDER BY created_at DESC LIMIT 30').all();
